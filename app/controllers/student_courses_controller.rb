class StudentCoursesController < ApplicationController 
    ...
    # def destroy
    #     course_to_remove = Course.find(params[:course_id]) #here the course is 7
    #     unless !current_user.courses.include?(course_to_remove)
            
    #         #test = course_to_remove.student_course.find(current_user.id) #(course.id = 7).studentcourse.find(student_id = 1)
    #         test = course_to_remove.student_courses.find(current_user.id)
    #         test.destroy
    ...
    #         flash[:notice] = "You have uncompleted #{course_to_add.name}!"
    #         redirect_to root_path
    #     else  
    #         flash[:notice] = "Something was wrong with the score update"
    #         redirect_to root_path
